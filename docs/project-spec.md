# Localhost Project Specification

For interview puproses i want to understand how internet works from the server side.

The Hypertext Transfer Protocol was created in order to ensure a reliable way to communicate on a request/response base.
...
For interview puproses I want to will learn the basics of the protocol and a good place to start could be the HTTP/1.1 RFC.
...
Your goal is to write your very own server, which will use HTTP protocol to serve static web pages to browsers.
...
- Based on the file extension the server will execute the corresponding CGI (for example .py or golang).
